...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- โค้ดปุ่มขยายรูป ---
  const displayButton = document.querySelector(".display-button button ");
  ...
  displayButton.addEventListener("click", () => {
    ...
    // accessibility
    const pressed = displayButton.getAttribute("aria-pressed") === "true";
    displayButton.setAttribute("aria-pressed", !pressed);

      images[0].scrollIntoView({ behavior: "smooth", block: "start" });

  // เพิ่ม scroll ลงอีก 100px หลังจากเลื่อน (ใช้ timeout เพื่อรอ scroll แรก)
  setTimeout(() => {
